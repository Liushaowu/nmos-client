...
public:
  ...
  make_node_implementation_connection_activation_handler() {
    return [&](const nmos::resource &resource,
      ...
      std::string resource_json = connection_resource.data.serialize();
      ...
      if (resource.type == nmos::types::receiver) {
        ...
        VideoReceiver *video = find_video_receiver_by_receiver_id(resource.id);
        AudioReceiver *audio = find_audio_receiver_by_receiver_id(resource.id);
        if (video) {
          ...
          if (update_video_receiver_func) {
            ...
            slog::log<slog::severities::info>(*gate_, SLOG_FLF)
                << nmos::stash_category(impl::categories::node_implementation)
                << "update video receiver callback success " << video->ip << ":"
                << video->port;
          } else {
        ...
        if (audio) {
          ...
          if (update_audio_receiver_func) {
            ...
            slog::log<slog::severities::info>(*gate_, SLOG_FLF)
                << nmos::stash_category(impl::categories::node_implementation)
                << "update audio receiver callback success " << audio->ip << ":"
                << audio->port;
            ;
          } else {
            slog::log<slog::severities::info>(*gate_, SLOG_FLF)
      ...
      const auto validate_sdp_parameters =
          [](const web::json::value &receiver,
            ...
            if (nmos::media_types::video_jxsv ==
              ...
              // }
              // nmos::sdp_parameters::ts_refclk_t trt;
              // trt.clock_source = sdp::timestamp_reference_clock_sources::ptp;
              // trt.ptp_version=sdp::ptp_versions::IEEE1588_2008;
              // trt.ptp_server="";
              // sdp_params.ts_refclk.emplace_back(trt);
            }
          };
  ...
  make_node_implementation_auto_resolver(const nmos::settings &settings) {
    ...
    return [&](const nmos::resource &resource,
      ...
      if (id_type.second == nmos::types::sender) {
        ...
        std::string interface_ip = "";
        VideoReceiver *video =
            find_video_receiver_by_receiver_id(connection_resource.id);
        if (video) {
          source_ip = "";
          port = video->port;
          smpte2022_7 = video->redudancy.enable;
          ip = video->ip;
          interface_ip = video->source_ip;
          redudancy_ip = video->redudancy.ip;
        }
        AudioReceiver *audio =
            find_audio_receiver_by_receiver_id(connection_resource.id);
        if (audio) {
          smpte2022_7 = audio->redudancy.enable;
          interface_ip = audio->source_ip;
          source_ip = "";
          ip = audio->ip;
          port = audio->port;
          redudancy_ip = audio->redudancy.ip;
        ...
        auto sdp_params = [&] {
          ...
          if (nmos::formats::video == format) {
            ...
            return nmos::make_audio_sdp_parameters(
                node->data, source->data, flow->data, sender.data,
                nmos::details::payload_type_audio_default, mids, {},
                packet_time);
  ...
  void init_device() {
    ...
    seed_id_ = seed_id;
    slog::log<slog::severities::info>(*gate_, SLOG_FLF)
        << "Node ID: " << node_id << " gmid: " << gmid
        << " ptp_locked: " << ptp_locked;
    const auto clocks = web::json::value_of({nmos::make_ptp_clock(
        nmos::clock_names::clk0, false, gmid, ptp_locked)});
  ...
  void add_video_sender(VideoSender video) {
    ...
    nmos::rational frame_rate = nmos::parse_rational(
        web::json::value_of({{nmos::fields::numerator, video.fps},
    ...
    nmos::interlace_mode interlace_mode =
        video.interlace ? nmos::interlace_modes::interlaced_tff
                        : nmos::interlace_modes::progressive;
  ...
  */
  ...
  void add_video_receiver(VideoReceiver video) {
    ...
    nmos::write_lock lock = node_model_.write_lock();
    bool smpte2022_7 = video.redudancy.enable;
    std::string id = video.id;
    std::string name = video.name;
    bool ST_2022_7 = video.redudancy.enable;
    nmos::rational frame_rate = nmos::parse_rational(web::json::value_of(
    ...
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::multicast_ip] = value(video.ip);
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::interface_ip] = value(video.source_ip);
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::destination_port] = video.port;
  ...
  void add_audio_receiver(AudioReceiver audio) {
    ...
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::multicast_ip] = value(audio.ip);
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::interface_ip] = value(audio.source_ip);
    connection_receiver
        .data[nmos::fields::endpoint_active][nmos::fields::transport_params][0]
             [nmos::fields::destination_port] = audio.port;
    if (ST_2022_7) {
      connection_receiver.data[nmos::fields::endpoint_constraints][1]
                              [nmos::fields::interface_ip] =
          value_of({{nmos::fields::constraint_enum,
      ...
      connection_receiver
          .data[nmos::fields::endpoint_active][nmos::fields::transport_params]
               [1][nmos::fields::multicast_ip] = value(audio.ip);
      connection_receiver
          .data[nmos::fields::endpoint_active][nmos::fields::transport_params]
               [1][nmos::fields::interface_ip] = value(audio.source_ip);
      connection_receiver
          .data[nmos::fields::endpoint_active][nmos::fields::transport_params]
               [1][nmos::fields::destination_port] = audio.port;
    }
    audio.receiver_id = receiver_id;
  ...
  void remove_video_receiver(std::string id) {
    ...
    nmos::id receiver_id = video->receiver_id;
    ...
    remove_video_receiver_by_receiver_id(receiver_id);
  }
  ...
  void remove_audio_receiver(std::string id) {
    ...
    nmos::id receiver_id = audio->receiver_id;
    ...
    remove_resource_after(delay_millis, node_model_.connection_resources,
                          receiver_id, *gate_, lock);
    remove_audio_receiver_by_receiver_id(receiver_id);
  }
  ...
  void remove_audio_sender_by_sender_id(std::string id) {
    ...
    audio_senders.erase(
        std::remove_if(audio_senders.begin(), audio_senders.end(),
                       [&](const AudioSender &a) { return a.sender_id == id; }),
        audio_senders.end());
  }
  ...
  void remove_video_sender_by_sender_id(std::string id) {
    ...
    video_senders.erase(
        std::remove_if(video_senders.begin(), video_senders.end(),
                       [&](const VideoSender &v) { return v.sender_id == id; }),
        video_senders.end());
  }
  ...
  AudioReceiver *find_audio_receiver_by_receiver_id(std::string id) {
    if (audio_receivers.empty()) {
      return nullptr;
    }
    for (auto &audio : audio_receivers) {
      if (audio.receiver_id == id) {
        return &audio;
      }
    }
    return nullptr;
  }
  void remove_audio_receiver_by_receiver_id(std::string id) {
    if (audio_receivers.empty()) {
      return;
    }
  ...
  VideoReceiver *find_video_receiver_by_receiver_id(std::string id) {
    if (video_receivers.empty()) {
      return nullptr;
    }
    for (auto &video : video_receivers) {
      if (video.receiver_id == id) {
        return &video;
      }
    }
    return nullptr;
  }
  void remove_video_receiver_by_receiver_id(std::string id) {
    if (video_receivers.empty()) {
      return;
    }
