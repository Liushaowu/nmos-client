      ...
      if (resource.type == nmos::types::receiver)
      {
        ...
        CallbackDispatcher::ReceiverCallbacks callbacks;
        {
          ...
          if (video)
          {
            ...
            if (video->redundancy.present && has_secondary_transport_param)
            {
          ...
          if (audio)
          {
            ...
            if (audio->redundancy.present && has_secondary_transport_param)
            {
          ...
          if (ancillary)
          {
            ...
            if (ancillary->redundancy.present && has_secondary_transport_param)
            {
        ...
        if (video_snapshot)
        {
          if (callbacks.video)
          {
            callbacks.video(*video_snapshot);
          }
          else
          {
            slog::log<slog::severities::error>(*ctx.gate, SLOG_FLF)
        ...
        if (audio_snapshot)
        {
          if (callbacks.audio)
          {
            callbacks.audio(*audio_snapshot);
        ...
        {
          if (callbacks.ancillary)
          {
            callbacks.ancillary(*ancillary_snapshot);
            slog::log<slog::severities::info>(*ctx.gate, SLOG_FLF)
                << nmos::stash_category(impl::categories::node_implementation)
